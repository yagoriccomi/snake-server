 ...
 *
 * ESTE ARQUIVO NÃO LÊ CONFIGURAÇÃO. Ele é usado por dois programas: o
 * servidor web (`lib/logger.ts`, que monta o logger a partir do `env.ts`) e o
 * worker de limpeza (que tem o próprio esquema de ambiente). Importar o
 * `env.ts` daqui faria o worker exigir as variáveis do servidor web e morrer
 * ao iniciar — foi o que aconteceu até 2026-09-25. [#13][#20]
 */
...
function mascararErro(erro: Error, incluirStack: boolean): Record<string, unknown> {
  return {
    nome: erro.name,
    mensagem: limparTexto(erro.message),
    stack: incluirStack ? erro.stack : undefined,
  };
}
...
function mascararObjeto(
  ...
  for (const [chave, conteudo] of Object.entries(objeto)) {
    ...
    saida[chave] = mascararValor(conteudo, profundidade + 1, incluirStack);
  }
...
function mascararValor(valor: unknown, profundidade: number, incluirStack: boolean): unknown {
  ...
  if (valor instanceof Error) return mascararErro(valor, incluirStack);
  if (valor instanceof Date) return valor.toISOString();

  if (Array.isArray(valor)) {
    return valor.map((item) => mascararValor(item, profundidade + 1, incluirStack));
  }

  if (typeof valor === 'object') {
    return mascararObjeto(valor as Record<string, unknown>, profundidade, incluirStack);
  }
 ...
 * Sem pedir, a stack de um erro não entra: o seguro é o padrão.
 */
export function mascarar(
  contexto: Record<string, unknown>,
  incluirStack = false,
): Record<string, unknown> {
  return mascararObjeto(contexto, 0, incluirStack);
}

export interface OpcoesDoLogger {
  /** Abaixo deste nível, nada é escrito. */
  nivelMinimo: NivelDeLog;
  /** A stack de um erro só ajuda fora de produção; em produção, some. [#93] */
  incluirStack: boolean;
}

/**
 * Cria um logger com a configuração RECEBIDA — cada programa decide a sua.
 * O mascaramento é o mesmo para todos: não é configurável. [#20][#63]
 */
export function criarLogger(opcoes: OpcoesDoLogger) {
  function escrever(nivel: NivelDeLog, mensagem: string, contexto?: Record<string, unknown>): void {
    if (PESO_DOS_NIVEIS[nivel] < PESO_DOS_NIVEIS[opcoes.nivelMinimo]) return;

    const registro = {
      nivel,
      horario: new Date().toISOString(),
      mensagem: limparTexto(mensagem),
      ...(contexto ? mascarar(contexto, opcoes.incluirStack) : {}),
    };
