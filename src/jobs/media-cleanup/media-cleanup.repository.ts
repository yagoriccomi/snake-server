  ...
  return {
    async listarPendentes(limite) {
      const parametros = new URLSearchParams({
        select: 'id,provider,asset_ref,tentativas',
      ...
      // Provedor fora do enum conhecido (schema divergente de uma versão
      // futura) é motivo de PARAR, não de adivinhar qual API chamar com um
      // identificador que pode não ser dele. [#9]
